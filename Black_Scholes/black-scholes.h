...
void MonteCarlo(float *h_CallResult, float *h_CallConfidence, float *S,
		float *X, float *T, int OPT_N) {
			...
			#pragma omp parallel private(l_Random) shared(h_CallResult,h_CallConfidence, S, X, T, OPT_N) num_threads(8)
			{
				...
				for (int opt = 0; opt < OPT_N; opt++) {
					...
					for (int pos = 0; pos < RAND_N; pos++) {
						float callValue =
								std::max(0.0, Sval *exp(MuByT + VBySqrtT * l_Random[pos]) - Xval);
						val += callValue;
